...
- Showcase full-stack MLOps skills (Model → API → Docker → Cloud)
...
- **PyTorch** — Deep learning library used to train a digit recognition model (MNIST)
...
<img src="/static/images/SwaggerUI.png" alt="Swagger UI" width="400" height="300" />
...
### Verify if login and project setup are successful (Optional)
...
- GCP Project ID：mnist-api-469005
- Region：asia-east1
...
Upload the current directory (including the Dockerfile and code) to Google Cloud Build, where the image is built on Google’s servers and then automatically push to the specified Artifact Registry.
...
- ❌ **.dockerignore doesn't work** — I already add `venv/` in .dockerignore, but it didn't work. And then I delete the venv file directly.
- ❌ **Out-of-memory error** — default 512 MiB was too small for this cmd (gcloud run deploy); increased to 1GiB
- ❌ **Incorrect base64 input** — not using proper grayscale-encoded MNIST images
...
- 📦 Add support for image file uploads (not just base64)
- 🧠 Swap in more advanced models (ResNet, YOLO, etc.)
- 🔁 Add CI/CD using GitHub Actions to auto-deploy on push
- 🌍 Add frontend for real-time predictions (React/Streamlit)
...
[👉 View on GitHub](https://github.com/ChengZheWu/Image-Classification-AI-API-with-Docker-and-Cloud-Deployment)
